    ...
    return (
        <div className="pb-4">
            <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-2 max-w-[400px] mt-4">
                ...
                {query.length > 0 && hasSubmit && window.innerWidth > 1024 && <Button variant='secondary' disabled={isLoading || query.trim().length === 0} className="px-3 py-2 w-full sm:w-auto transition duration-300 ease-in-out lg:hover:scale-[103%]" onKeyDown={handleKeyDown} onClick={handleClear}>Clear</Button>}
            </div>
        </div>
    )
}
